...
assemble_utf8(char **inptr, int cnt) {
	unsigned char *in;
	unsigned int uc, uc1, uc2 = 0;
	int need, need1, bit, bad = 0;
	...
	if (bad) {
		char *u = _("%s: bad utf8 %02x %02x (need %i, cnt %i)\n");
		fprintf(stderr, u, progname, uc1, uc2, need1, cnt);
		exit(EX_DATAERR);
	}
	*inptr = in;
	return uc;
...
readpsffont(FILE *fontf, char **allbufp, int *allszp,
	    ...
	    int *fontlenp, int *fontwidthp, int fontpos0,
...
int
writepsffont(FILE *ofil, char *fontbuf, int width, int charsize, int fontlen,
	     int psftype, struct unicode_list *uclistheads, int fd) {
	int i;
	int utf8;
	int seqs = 0;
	int bpl = (width + 7) / 8;
	int height = charsize / bpl;

	if (uclistheads && uclistheads != (struct unicode_list*)-1)
		seqs = has_sequences(uclistheads, fontlen);

	/* Output new font file */
	if ((fontlen != 256 && fontlen != 512) || psftype == 2 || width != 8) {
