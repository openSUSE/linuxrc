/*
 *
 * net.h         Header file for net.c
 *
 * Copyright (c) 1996-2001  Hubert Mantel, SuSE GmbH  (mantel@suse.de)
 *
 */

extern int  net_config          (void);
extern int  net_bootp           (void);
extern int  net_mount_nfs       (char *server_addr_tv, char *hostdir_tv);
extern int  net_mount_smb       (void);
extern void net_smb_get_mount_options (char* options);
extern void net_stop            (void);
extern int  net_check_address   (char *input_tv, struct in_addr *address_prr);
extern int  net_check_address2  (inet_t *inet, int do_dns);
extern int  net_setup_localhost (void);

extern int  net_activate        (void);
extern int  net_is_configured_im;
extern int  net_dhcp            (void);
extern void net_dhcp_stop       (void);
extern unsigned net_config_mask (void);
extern int net_get_address(char *text, inet_t *inet, int do_dns);
